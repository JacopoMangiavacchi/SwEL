...
extension String {
    ...
    ///
    ///     example 1:  
    ///         try! "(1 == 2 || int(1 + 1) < 4) && 'test' != 'ko'".checkCondition()
    ///
    ///     example 2:  
    ///         try! "var1 > float(var2 * 2)".checkCondition(withVariables: ["var1" : 4.0, "var2" : 1.5])
    ///
    ///     example 3:
    ///         try! "substring('This is a test for testing regexp', 'test') == 'test'".checkCondition()
    ///
